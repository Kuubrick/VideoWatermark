...
def process_and_save_video(
    ...
    try:
        video = process_video(
            video_path, mask_path, img_size=img_size, wide_crop=wide_crop
        )
...
def process_video(
    video_path, mask_path, img_size, threshold=5, margin=0.02, wide_crop: bool = False
):
    ...
    for img, mask in zip(video_reader, mask_reader):
        ...
        try:
            dst_img = cv2.resize(
                dst_img, (img_size, img_size), interpolation=cv2.INTER_LANCZOS4
            ).transpose(2, 0, 1)
...
def main():
    ...
    parser.add_argument(
        "--source_dir", type=str, default="data/FaceForensics_compressed"
    )
    ...
    parser.add_argument(
        "--wide_crop",
        action="store_true",
        help="Should we crop each frame independently (this makes a video shaking)?",
    )
    ...
    for split in splits:
        ...
        for i, filename in enumerate(filenames):
            ...
            video_path = os.path.join(
                args.source_dir,
                fn_frame[fn_frame["category"] == "original"].iloc[0]["filepath"],
            )
            mask_path = os.path.join(
                args.source_dir,
                fn_frame[fn_frame["category"] == "mask"].iloc[0]["filepath"],
            )
            ...
            job_kwargs_list.append(
                dict(
                    ...
                    output_dir=os.path.join(
                        args.output_dir, filename[: filename.rfind(".")]
                    ),
                )
            )

        for _ in tqdm(
            pool.imap_unordered(job_proxy, job_kwargs_list),
            desc=f"Processing {split}",
            total=len(job_kwargs_list),
        ):
            pass
            # T = len(video)
