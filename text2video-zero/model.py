...
#     def inference_chunk(self, frame_ids, with_watermark, **kwargs):
...
#     def inference(self, with_watermark, split_to_chunks=False, chunk_size=8, **kwargs):
#         if not hasattr(self, "pipe") or self.pipe is None:
#             return
...
#             video_path, resolution, self.device, self.dtype, False
#         )
#         control = utils.pre_process_depth(video).to(self.device).to(self.dtype)
...
class Model:
    ...
    def inference_chunk(
        self, frame_ids, with_watermark, w_pattern, w_radius, li_radius=None, **kwargs
    ):
        if not hasattr(self, "pipe") or self.pipe is None:
            return
    ...
    def process_text2video(
        ...
        added_prompt = "high quality, HD, 8K, trending on artstation, high focus, dramatic lighting"
        negative_prompts = "longbody, lowres, bad anatomy, bad hands, missing fingers, extra digit, fewer difits, cropped, worst quality, low quality, deformed body, bloated, ugly, unrealistic"
        ref_model, _, ref_clip_preprocess = open_clip.create_model_and_transforms(reference_model, pretrained=reference_model_pretrain, device=self.device)
        ...
        if with_watermark:
            ...
            li_radius = utils.assign_radii(result, clip_scores)
            ...
            return utils.create_video(
                result, fps, path=path, watermark=None
            )
        else:
            result = self.inference(
                prompt=prompt,
