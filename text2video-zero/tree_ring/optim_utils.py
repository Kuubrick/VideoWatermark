...
def image_distortion(img1, img2, seed, args):
    ...
    if args.jpeg_ratio is not None:
        img1.save(f"tmp_{args.jpeg_ratio}_img1.jpg", quality=args.jpeg_ratio)
        img1 = Image.open(f"tmp_{args.jpeg_ratio}_img1.jpg")
        img2.save(f"tmp_{args.jpeg_ratio}_img2.jpg", quality=args.jpeg_ratio)
        img2 = Image.open(f"tmp_{args.jpeg_ratio}_img2.jpg")
...
def get_watermarking_pattern(pipe, args, device, shape=None):
    ...
    if 'seed_ring' in args.w_pattern:
        ...
        gt_patch_tmp = copy.deepcopy(gt_patch) #随机生成的gt_patch_tmp
        for i in range(args.w_radius, 0, -1):
            tmp_mask = circle_mask(gt_init.shape[-1], r=i)
            tmp_mask = torch.tensor(tmp_mask).to(device)#随机生成的圆形掩膜，在(0,radius]中遍历,所谓掩膜就是把选中的圈圈置为True
...
def eval_watermark(reversed_latents_no_w, reversed_latents_w, watermarking_mask, gt_patch, args):
    ...
    if 'l1' in args.w_measurement:
        no_w_metric = torch.abs(reversed_latents_no_w_fft[watermarking_mask] - target_patch[watermarking_mask]).mean().item() # l1-distance
        w_metric = torch.abs(reversed_latents_w_fft[watermarking_mask] - target_patch[watermarking_mask]).mean().item()# l1-distance
    else:
        NotImplementedError(f'w_measurement: {args.w_measurement}')
