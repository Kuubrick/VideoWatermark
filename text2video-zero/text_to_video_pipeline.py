...
#         def get_watermarking_pattern(w_radius, device, shape=None):
...
#                 # 随机生成的圆形掩膜，在(0,radius]中遍历,所谓掩膜就是把选中的圈圈置为True
#                 tmp_mask = circle_mask(gt_init.shape[-1], r=i)
#                 tmp_mask = torch.tensor(tmp_mask).to(device)

#                 for j in range(gt_patch.shape[1]):
#                     # gt_patch是torch.Size([1, 4, 64, 64])，通过j遍历所有通道，通过tmp_mask选出所有需要改变的值，统一置为当前通道圈上的一个值
...
#         ##############################################################################
#         def inject_watermark(init_latents_w, watermarking_mask, gt_patch):
#             init_latents_w_fft = torch.fft.fftshift(
#                 torch.fft.fft2(init_latents_w), dim=(-1, -2)
#             )
#             # 把init_latents_w_fft[此时还是干净的]中间的watermarking_mask置为gt_patch里的watermarking_mask值
...
#         )
#         # ddim_res是一个字典，包含了x0, x_t0_1, x_t1_1
#         x0 = ddim_res["x0"].detach()
...
#                 w_radius, device, shape=tmp_x_t0_1.shape
#             )
#             watermarking_mask = get_watermarking_mask(tmp_x_t0_1, w_radius, device)
#             x_t0_1[:, :, 0] = inject_watermark(
#                 tmp_x_t0_1, watermarking_mask, gt_patch
#             ).clone()
#             # x_t1_1[:,:,0] = inject_watermark(tmp_x_t1_1, watermarking_mask, gt_patch).clone()
#         ############################################################################
#         del ddim_res
#         del xT
#         if use_motion_field:
...
#             # x0是所有的DDIM_backward的结束后的latent
#             x0 = ddim_res["x0"].detach()
#             del ddim_res
#             del x_t1
#             del x_t1_1
...
class TextToVideoPipeline(StableDiffusionPipeline):
    ...
    def __call__(
        ...
        **kwargs,
        ...
        def get_watermarking_pattern(w_radius, device, w_pattern, shape=None):
            if shape is not None:
                gt_init = torch.randn(*shape, device=device)
            else:
                gt_init = self.prepare_latents(
                ...
                for w_radius in range(16, 9, -1):
                    ...
                    for i in range(w_radius, 0, -1):
                        ...
                        for j in range(gt_patch.shape[1]):
                            # gt_patch是torch.Size([1, 4, 64, 64])，通过j遍历所有通道，通过tmp_mask选出所有需要改变的值，统一置为当前通道圈上的一个值
                            gt_patch[:, j, tmp_mask] = gt_patch_tmp[0, j, 0, i].item()
                    dic_gt_patch.update({w_radius: gt_patch})
                    gt_patch = copy.deepcopy(gt_patch_origin)

            elif "zeros" in w_pattern:
                for w_radius in range(16, 9, -1):
                    gt_patch = (
                        torch.fft.fftshift(torch.fft.fft2(gt_init), dim=(-1, -2)) * 0
                    )
                    dic_gt_patch.update({w_radius: gt_patch})
            return dic_gt_patch
        ...
        ##############################################################################
        def inject_watermark(init_latents_w, watermarking_mask, gt_patch, w_pattern):
            init_latents_w_fft = torch.fft.fftshift(
                torch.fft.fft2(init_latents_w), dim=(-1, -2)
            )
            # 把init_latents_w_fft[此时还是干净的]中间的watermarking_mask置为gt_patch里的watermarking_mask值
            if "zeros" in w_pattern:
                init_latents_w_fft[watermarking_mask] = 0 + 0j
            else:
                init_latents_w_fft[watermarking_mask] = gt_patch[
                    watermarking_mask
                ].clone()
            init_latents_w = torch.fft.ifft2(
                torch.fft.ifftshift(init_latents_w_fft, dim=(-1, -2))
        ...
        #     tmp_x_t1_1 = x_t1_1[:, :, 0]
        #     gt_patch = get_watermarking_pattern(
        #         w_radius, device, w_pattern=w_pattern, shape=tmp_x_t0_1.shape
        #     )
        #     watermarking_mask = get_watermarking_mask(tmp_x_t0_1, w_radius, device)
        #     x_t0_1[:, :, 0] = inject_watermark(
        #         tmp_x_t0_1, watermarking_mask, gt_patch, w_pattern
        #     ).clone()
        #     # x_t1_1[:,:,0] = inject_watermark(tmp_x_t1_1, watermarking_mask, gt_patch).clone()
        ############################################################################
        del ddim_res
