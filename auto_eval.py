...
def get_clip(
    eval_model: Module, device: Union[torch.device, int]
) -> Tuple[Module, Module]:
    """Get the CLIP model
...
def init_metric(
    ...
    """
    m = metric(768 if eval_model == "ViT-L/14" else 512, limit=limit)
    m.cuda(device)
    m._debug = False
    return m
...
def populate_metrics(
    ...
    for i, (fake, caption, vid) in enumerate(tqdm(dataloader)):
        ...
        fake = fake.reshape(
            -1, fake.shape[-3], fake.shape[-2], fake.shape[-1]
        )  # [batch_size * frame_num, 3, h, w]
        ...
        fake_im_features = clip_model.encode_image(
            fake
        ).float()  # [batch_size * frame_num, hidden_size]
        ...
        Y_ref = txt_features
        Y_ref = txt_features
        X = fake_im_features.reshape(
            batch_size, fake_frame_num, fake_im_features.shape[-1]
        )  # [batch_size, frame_num, hidden_size]
...
def blipscore(config_path, args):
    ...
    test_dataset = VideoDataset(
        ...
        media_size=config["image_size"],
        max_num_frm=config["num_frm_test"],
    )
    ...
    return compute_blipscore(
        model_without_ddp,
        test_loader,
        model_without_ddp.tokenizer,
        args.device,
        config,
        utils,
    )
...
@torch.no_grad()
def compute_blipscore(
    model, data_loader, tokenizer, device, config, utils, use_itm=True
):
    """
    ...
    if use_itm:
        ...
        for i in tqdm(range(0, num_text, config["batch_size"])):
            encoder_output = video_feats[
                i : min(num_text, i + config["batch_size"])
            ].to(device, non_blocking=True)
            encoder_att = torch.ones(encoder_output.size()[:-1], dtype=torch.long).to(
                device, non_blocking=True
            )
            output = model.text_encoder(
                text_ids[i : min(num_text, i + config["batch_size"])],
...
if "__main__" == __name__:
    ...
    parser.add_argument(
        ...
        default=False,
        type=str2bool,
        help="Whether the CLIP model is fine-tuned or not, set to True when computing CLIPScore-ft",
    )
    ...
    parser.add_argument(
        "--gen_path",
        default=None,
        help="path to the generated videos, processed into frames",
    )
    parser.add_argument(
        "--t2v_model",
        choices=[
            ...
            "text2video-zero-10",
            "text2video-zero-16",
            "text2video-zero-dynamic",
        ],
        required=True,
        help="Name of the text2video generation model",
    )
    parser.add_argument("--seed", default=42, type=int)
    parser.add_argument("--max_frm_num", default=64, type=int)
    parser.add_argument(
        "--limit", default=1000, type=int, help="number of sampels to evaluate"
    )
    ...
    metrics = [
        init_metric("datas", x, args.eval_model, args.limit, args.device)
        for x in METRICS
    ]
    ...
    ds = VideoDataset(
        gen_path=args.gen_path,
        prompt_file=args.prompt_file,
        max_num_frm=args.max_frm_num,
        preprocessor=clip_prep,
    )
    dl = DataLoader(ds, batch_size=16, drop_last=False, shuffle=False, num_workers=0)
    ...
    if args.save_results:
        for metric in results_per_sent_id:
            ...
            result_file = os.path.join(
                result_path, f"auto_eval_results_{args.t2v_model}.json"
            )
